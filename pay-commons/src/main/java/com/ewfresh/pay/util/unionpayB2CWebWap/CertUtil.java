...
import static com.ewfresh.pay.util.unionpayb2cwebwap.SDKConstants.UNIONPAY_CNNAME;
import static com.ewfresh.pay.util.unionpayb2cwebwap.SDKUtil.isEmpty;
...
public class CertUtil {
	...
	private static void initMiddleCert() {
		...
		logger.info("加载中级证书==>"+middleCertPath);
		if (!isEmpty(middleCertPath)) {
			middleCert = initCert(middleCertPath);
			LogUtil.writeLog("Load MiddleCert Successful");
		} else {
			logger.error("WARN: acpsdk.middle.path is empty");
		}
	}
	...
	private static void initRootCert() {
		...
		if (!isEmpty(SDKConfig.getConfig().getRootCertPath())) {
			...
			logger.warn("WARN: acpsdk.rootCert.path is empty");
		}
	}
	...
	private static void initEncryptCert() {
		...
		if (!isEmpty(SDKConfig.getConfig().getEncryptCertPath())) {
			...
			logger.error("WARN: acpsdk.encryptCert.path is empty");
		}
	}
	...
	private static X509Certificate initCert(String path) {
		...
		try {
			cf = CertificateFactory.getInstance("X.509", "BC");
			//根据相对路径拿到证书的绝对路径
			path = CertUtil.class.getResource(path).toURI().getPath();
			...
			logger.error("Relation path to Absolute path occurred URISyntaxException", e);
		} finally {
			if (null != in) {
				try {
					in.close();
	...
	private static KeyStore getKeyInfo(String pfxkeyfile, String keypwd,
		...
		try {
			//根据相对路径拿到证书的绝对路径
			String pfxKeyFile = CertUtil.class.getResource(pfxkeyfile).toURI().getPath();
	...
	private static boolean verifyCertificateChain(X509Certificate cert){
		...
		try {
            ...
            PKIXCertPathBuilderResult result = (PKIXCertPathBuilderResult) builder
                .build(pkixParams);
        	logger.error("verify certificate chain succeed.");
			return true;
