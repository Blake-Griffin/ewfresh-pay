...
public class ParseXMLUtil {
	 ...
	 * */
	public HashMap returnXMLDataList(Element root,String flag1,String flag2,String flag3,String flag4){
		HashMap xmlData=null;	//定义一个HashMap对象
		Element child=null;			//定义一个Element元素对象
		...
		if(root!=null){
			...
			for (Iterator childs1= root.getChildren().iterator();childs1.hasNext();) {
				...
				} else if (childName.equals(flag3)) {
					xmlData=parseXMLNodeList(xmlData,child,flag4);   //调用parseXMLNode函数，得到该节点下的所有元素
				} else {
					if ("ErrorMsgContent".equals(childName)) {
						xmlData = parseXMLNode(xmlData, child);   //调用parseXMLNode函数，得到该节点下的所有元素
					} else {
						xmlData.put(childName, child.getValue());  //将子元素保存到HashMap中
					}
